...
namespace CustomPreLaunchChecks
{
    ...
    {
        internal static HashSet<IntPtr> allDetours = new HashSet<IntPtr>();
        ...
        public class Detour
        {
            ...
            public unsafe Detour(MethodBase oldFunction, MethodBase newFunction)
            {
                ...
                if (allDetours.Contains(oldFunctionPointer))
                {
                    Log.Error("Cannot install new Function, because someone else is using it: " + oldFunction.Name + " " + newFunction.Name);
                }

                allDetours.Add(oldFunctionPointer);
